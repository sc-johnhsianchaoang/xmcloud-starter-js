function toPlainText(raw: unknown): string {
  ...
  if (typeof raw === "object") {
    ...
    if (fields.jsonValue) {
      return toPlainText(
        (fields.jsonValue as Record<string, unknown>)?.value ??
          fields.jsonValue,
      );
    }
    for (const key of [
...
function extractFieldText(
  ...
  const isHeading = headingNames.some((candidate) =>
...
function parseRendering(
  ...
  const componentName =
    (node.componentName as string) || (node.name as string) || "";
  const componentHeading = formatText(componentName);
  if (componentHeading && headingLevel <= 3) {
    appendUnique(
      lines,
      `${"#".repeat(Math.min(3, headingLevel))} ${componentHeading}`,
    );
  }
  ...
  if (placeholders && typeof placeholders === "object") {
    Object.values(placeholders).forEach((placeholder) => {
      if (Array.isArray(placeholder)) {
        placeholder.forEach((child) =>
          parseRendering(child, headingLevel + 1, lines),
        );
      }
    });
  }
  ...
  Object.values(placeholders).forEach((placeholder) => {
    if (Array.isArray(placeholder)) {
      placeholder.forEach((rendering) =>
        parseRendering(rendering, headingLevel, lines),
      );
...
export function generateMarkdownFromRoute(route: unknown): string {
  ...
  const description = formatText(
    routeFields?.metadataDescription ??
      routeFields?.pageSummary ??
  ...
  const renderings = Array.isArray(routeObj.renderings)
    ? routeObj.renderings
    : undefined;
  if (renderings) {
